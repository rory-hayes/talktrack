...
| Favorites / starred prompts | `UserDefaults` set of scenario IDs on the current device | Local device only | Local only | Device-local only | Readable and writable offline | Starred prompts stay on this device | `ios/Clearify/Sources/Services/FavoriteScenarioStore.swift`, `ios/Clearify/Sources/ViewModels/ScenarioLibraryViewModel.swift` |
| Saved answers | `UserDefaults` saved-answer IDs plus local archive session data and copied local audio | Local device only | Local only | Device-local only | Readable and writable offline if the archived session/audio exists locally | Saved answers stay in the local archive on this device | `ios/Clearify/Sources/Services/SavedAnswerStore.swift`, `ios/Clearify/Sources/Services/LocalPracticeStore.swift`, `ios/Clearify/Sources/ViewModels/ProgressViewModel.swift`, `ios/Clearify/Sources/Views/Progress/ProgressViewScreen.swift` |
...
| Local archive | Device-local JSON archive plus copied audio files in app support directory | Local device only | Local only | Device-local only | Readable and writable offline | No direct user promise; this is a fallback/store of device-local history | `ios/Clearify/Sources/Services/LocalPracticeStore.swift` |
...
- Saved answers depend on both the saved-answer ID set and the retained local session/audio archive.
